<template>
    <section>
        ...
        <nav-bread>
            <span slot="pathName">OrderConfirm</span>
        </nav-bread>
        <div>
                ...
                <div class="checkout-order">
                    ...
                    <div class="item-list-wrap confirm-item-list-wrap">
                        <div class="cart-item order-item">
                            ...
                            <ul class="cart-item-list">
                                <li v-for="item in selectedList" :key="item.productId">
                    ...
                    <div class="order-foot-wrap">
                        <div class="prev-btn-wrap">
                            <button class="btn btn--m"><router-link to="address">Previous</router-link></button>
                        </div>
                        <div class="next-btn-wrap">
                            <button class="btn btn--m btn--red" @click="createOrder" :class="{'btn--dis':selectedList.length==0}">Proceed to payment</button>
                        </div>
                    </div>
                </div>
            </div>
  ...
  computed: {
    subTotal () { // 订单中商品总价
      ...
      this.selectedList.forEach((item) => {
        total += item.productNum * (item.salePrice / 1)
      })
      return total
    },
    tax () { // 税
  ...
  methods: {
    ...
    createOrder () { // 调接口创建订单
      ...
      axios.post('users/createOrder', {
        ...
        if (res.data.status === '0' && this.selectedList.length > 0) {
          this.$router.push({
            path: '/orderSuccess',
            query: {
