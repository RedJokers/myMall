<template>
    <section>
                ...
                <div class="item-list-wrap">
                    <div class="cart-item">
                        ...
                        <ul class="cart-item-list">
                            <li v-for="product in cartList" :key="product.productId">
                                <div class="cart-tab-1">
                                    <div class="cart-item-check">
                                        <a href="javascript:;" class="checkbox-btn item-check-btn" :class="{'checked': product.checked==1}" @click="editCart('check',product)">
                                    ...
                                    <div class="cart-item-pic">
                                        <img :src="`/static/${product.productImage}`">
                                    </div>
                                    ...
                                    <div class="item-quantity">
                                        <div class="select-self select-self-open">
                                            <div class="select-self-area">
                                                <a class="input-sub" @click="editCart('subtract', product)">-</a>
                                                <span class="select-ipt">{{ product.productNum }}</span>
                                                <a class="input-add" @click="editCart('add', product)">+</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="cart-tab-5">
                                    <div class="cart-item-opration">
                                        <a href="javascript:;" class="item-edit-btn"  @click="delCartConfirm(product.productId)">
        ...
        <modal :md-show="delCartMd" @close="closeModal">
...
export default {
  ...
  data () {
    return {
      delCartMd: false, // 控制删除商品模态框显示隐藏
      productId: '', // 删除商品时所保存的参数
      cartList: [] // 购物车列表
    }
  },
  computed: {
  ...
  methods: {
    ...
    editCart (opration, product) { // 购物车商品数量增加和删除功能
      if (opration === 'add') { // 商品数量增加
        product.productNum++
      } else if (opration === 'subtract') { // 商品数量减少
        if (product.productNum / 1 === 1) {
          return
        }
        product.productNum--
      } else { // 选中商品
    ...
    toggleCheckAll () { // 选中全部
      ...
      axios.post('/users/editCart/checkAll', {
        checkAll: flag ? '1' : '0'
      }).then((res) => {
        if (res.data.status === '0') {
