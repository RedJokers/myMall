<template>
    <div>
        ...
        <nav-bread>
            <span slot="pathName">Address</span>
        </nav-bread>
            ...
            <div class="container">
                <div class="checkout-addr">
                    ...
                    <div class="addr-list-wrap">
                        ...
                        <div class="shipping-addr-more" @click="expand" v-if="this.addressList.length > 3">
                    ...
                    <div class="next-btn-wrap">
                        <a href="javascript:;" class="btn btn--m btn--red" @click="nextPage">Next</a>
                    </div>
                </div>
            </div>
        </div>
        <nav-footer></nav-footer>
        <modal :md-show="delAddressShow" @close="closeModal">
...
export default {
  ...
  data () {
    return {
      limit: 3, // 默认显示至多3条地址
      checkIndex: -1, // 设置地址是否选中
      addressList: [],
      delAddressShow: false, // 控制模态框的显示隐藏
      delAddressId: '', // 保存要删除的地址ID
      selectedAddrId: ''
    }
  },
  computed: {
    addressListFilter () { // 计算显示多少地址
  ...
  methods: {
      ...
      axios.get('/users/addressList').then((res) => {
        if (res.data.status === '0') {
          ...
          for (let i = 0; i < this.addressList.length; i++) {
            ...
            // // 将默认地址设置到最前面
            // let defaultAdress = this.addressList[i]
            // this.addressList.splice(i, 1)
            // this.addressList.unshift(defaultAdress)
              this.checkIndex = i
    ...
    setDefault (addressId) { // 设置默认地址
      axios.post('/users/setDefaultAddress', {addressId}).then((res) => {
        if (res.data.status === '0') {
          this.getAddressList()
        }
      })
    ...
    closeModal () { // 关闭模态框
      this.delAddressShow = false
    },
    delAddressConfirm (addressId) { // 删除地址确认
      this.delAddressShow = true
      this.delAddressId = addressId
    },
    delAddress () { // 删除地址
      axios.post('/users/delAddress', {addressId: this.delAddressId}).then((res) => {
        if (res.data.status === '0') {
          this.delAddressShow = false
          location.reload()
        }
